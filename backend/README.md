...
|--------|------|-------------|
| GET | `/` | Landing page with username search form (mode toggle: Challenge / cEDH) |
...
The "Build a cEDH Deck" feature relies on a reference corpus at
`src/data/cedh-corpus.json`. This file is **not** produced by `npm run build`
(the regular build only compiles TypeScript and copies `src/data` → `dist/data`).
It is generated by a separate script that scrapes Moxfield via Puppeteer:

```bash
npm run build:cedh          # generate/refresh src/data/cedh-corpus.json
git add src/data/cedh-corpus.json && git commit -m "Refresh cEDH corpus"
```

Commit the generated file so it ships with the Docker image — the Dockerfiles
`COPY src` (which includes `src/data`) and `npm run build` copies it into
`dist/data`, so no extra deploy step is needed at build time. Re-run periodically
to keep prices and the cEDH metagame current. A committed placeholder (empty
corpus) is included so the app builds and runs even before the script is first run.

> **Prices & FX:** Card prices are captured from Moxfield at corpus-build time
> (the printing used in each reference decklist) and stored in the corpus. The
> USD→CAD rate is fetched at runtime from a free FX API and cached globally for
> 24 hours, so pricing needs no extra API keys or per-request price calls.
